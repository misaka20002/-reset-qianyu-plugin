...
//import Config from '../../../models/base/Config.js'
class pluginLoader {
    constructor() {
        this.apps = [] //插件功能列表
        this.priority = [] //任务队列
    ...
    dealMsg(e) {
        ...
        // if (e.user_id && Config.GetCfg('qq').masterQQ.includes(Number(e.user_id))) {
        //     e.isMaster = true
        // }
    ...
    watch(dirName, appName) {
        ...
        let file = `./plugins/qianyu-plugin/apps/${appName}`
    ...
    watchDir(dirName) {
        ...
        let file = `./plugins/qianyu-plugin/${dirName}/`
        const watcher = chokidar.watch(file)
